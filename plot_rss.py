...
STEP_ORDER = [
    # "request_queue_wait",  # ensure this appears first in duration plots
    "generate_embeddings",
...
def read_occurrences(metrics_path: str) -> List[Dict[str, Any]]:
    occurrences = []
    ...
    occurrences.sort(key=lambda x: x.get("timestamp", 0))
    return occurrences
...
def main():
    ...
    ap.add_argument('--metrics-file', default='metrics.jsonl', help='Path to metrics.jsonl occurrences file')
    ...
    occurrences = read_occurrences(args.metrics_file)
    if not occurrences:
        print(f"No occurrence records found in {args.metrics_file}")
        return
